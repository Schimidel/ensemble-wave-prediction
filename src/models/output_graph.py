...
def historical_error(output, save_path, df):
    ...
    # Carrega os dados
    df_target = pd.read_csv(f'./data/processed/{name}/noaa_data_target.csv', 
                          encoding='utf-8', sep=';', decimal=',').drop('Unnamed: 0', axis=1)
    df_rel = pd.read_csv(f'./data/processed/{name}/noaa_data_relative.csv',
                        encoding='utf-8', sep=';', decimal=',').drop('Unnamed: 0', axis=1)
    
    # DEBUG - Verificação da estrutura dos dados
    print("\n=== DEBUG ===")
    print(f"df.shape: {df.shape}")  # (64, 6)
    print(f"df_target.shape (original): {df_target.shape}")  # (8, 64)
    print(f"df_rel.shape (original): {df_rel.shape}")  # (8, 64)
    
    # CORREÇÃO: Transpõe os DataFrames para ficarem com 64 linhas
    df_target = df_target.T  # Agora (64, 8)
    df_rel = df_rel.T  # Agora (64, 8)
    print("\nDataFrames transpostos:")
    print(f"df_target.shape: {df_target.shape}")
    print(f"df_rel.shape: {df_rel.shape}")
    print()
    print(df)
    print(df_target)
    print(df_rel)
    ...
    if df_target.shape[0] != df.shape[0] or df_rel.shape[0] != df.shape[0]:
        raise ValueError(f"Incompatibilidade de linhas: df tem {df.shape[0]}, df_target tem {df_target.shape[0]}, df_rel tem {df_rel.shape[0]}")

    # CORREÇÃO: Ajuste do lead_lists para ter 64 elementos
    # Original: lead_lists = [i for i in range(3, df_target.shape[1]*3+1, 3)] (8 elementos)
    # Nova versão:
    lead_lists = np.linspace(3, df_target.shape[1]*3, num=df.shape[0], dtype=int)
    print(f"\nLead lists shape: {len(lead_lists)} (deve ser {df.shape[0]})")

    with open(f'./data/processed/{name}/boia.pkl', 'rb') as handle:
        boia = pickle.load(handle)
    df_boia = pd.read_csv(boia).set_index('Datetime')

    # Adiciona dados da boia
    ...
    # Atribuições corrigidas (agora com shapes compatíveis)
    df['noaa_mean_error'] = df_target.mean(axis=1).values  # 64 valores
    df_target = np.abs(df_target)
    df['noaa_mean_error_abs'] = df_target.mean(axis=1).values  # 64 valores
    df['noaa_mean_error_rel'] = df_rel.mean(axis=1).values  # 64 valores
    
    # CORREÇÃO: Atribuição corrigida do Lead
    df['Lead'] = lead_lists  # Agora com 64 valores
    
    # Gera o gráfico
    plt.figure()
    plt.plot(df['Lead'], df['noaa_mean_error_rel']*100, '--', 
             label='NOAA historical relative error', color='black')
    plt.plot(df['Lead'], df['error_rel']*100, 
             label='NN mean - this work relative error')
    plt.ylabel('Relative error - $\Delta_{rel}$ (%)')
    plt.xlabel('Lead time')
    plt.legend()
    
       # Garantia de criação de pasta
    os.makedirs(save_path, exist_ok=True)
    os.system(f"chmod 777 {save_path}")
    
    # Caminho absoluto garantido
    full_path = os.path.join(save_path, 'historical_errors.png')
    print(f"\n🛠️ Tentando salvar em: {full_path}")
    ...
    try:
        ...
        if os.path.exists(full_path):
            print(f"📄 Arquivo encontrado! Tamanho: {os.path.getsize(full_path)} bytes")
        else:
            print("❌ Arquivo não encontrado após salvamento!")
