...
def multi_target_setup(path):
    """Versão com diagnóstico completo dos arquivos no diretório"""
    import os
    ...
    # 2. Listagem completa dos arquivos (com diagnóstico)
    print("\n[DIAGNÓSTICO] Conteúdo do diretório:")
    print(f"Caminho absoluto: {os.path.abspath(path)}")
    print("Arquivos encontrados:")
    for i, f in enumerate(sorted(os.listdir(path)), 1):
        file_path = os.path.join(path, f)
        print(f"{i}. {f} (Tamanho: {os.path.getsize(file_path)} bytes)")
    
    # 3. Busca específica pelos arquivos necessários
    required_files = {
        'first_hour_predict.pkl': None,
        'noaa_data_features.csv': None, 
        'noaa_data_target.csv': None
    }
    
    for f in os.listdir(path):
        if f in required_files:
            required_files[f] = os.path.join(path, f)
    
    # 4. Verificação dos arquivos obrigatórios
    missing_files = [k for k, v in required_files.items() if v is None]
    if missing_files:
        raise FileNotFoundError(
            f"Arquivos obrigatórios não encontrados: {missing_files}\n"
            f"Arquivos encontrados: {os.listdir(path)}"
        )
    
    # 5. Carregamento dos arquivos (com tratamento de erros)
    try:
        with open(required_files['first_hour_predict.pkl'], 'rb') as f:
            first_predict_date = pickle.load(f)
            print(f"\nData de primeira previsão carregada: {first_predict_date}")
    except Exception as e:
        raise ValueError(f"Erro ao carregar first_hour_predict.pkl: {str(e)}")
    
    try:
        df_feat = pd.read_csv(
            required_files['noaa_data_features.csv'],
            encoding='utf-8',
            sep=';',
            decimal=','
        ).drop('Unnamed: 0', axis=1)
        print(f"Features carregadas. Shape: {df_feat.shape}")
    except Exception as e:
        raise ValueError(f"Erro ao carregar features: {str(e)}")
    
    try:
        df_target = pd.read_csv(
            required_files['noaa_data_target.csv'],
            encoding='utf-8',
            sep=';',
            decimal=','
        ).drop('Unnamed: 0', axis=1)
        print(f"Targets carregados. Shape: {df_target.shape}")
    except Exception as e:
        raise ValueError(f"Erro ao carregar targets: {str(e)}")
    
    # 6. Geração das datas de previsão
    pred_dates = pd.date_range(
        start=first_predict_date,
        periods=df_feat.shape[1],
        freq='3H'
    )
